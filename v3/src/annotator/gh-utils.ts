export class GithubClient {
    ...
    async getFile(acct:string, repo:string, path:string, ref:string): Promise<any> {
      ...
      if (ref) url += `?ref=${ref}`
      if (!this.authToken) return Promise.resolve({content, sha: ''})
    ...
    */
    ...
    async getFile(acct:string, repo:string, path:string, branch:string): Promise<any> {
      let source = (window as any).config?.source || {}
      acct = acct || source.owner
      repo = repo || source.repository
      branch = branch || source.branch
      // console.log(`getFile: acct=${acct} repo=${repo} branch=${branch} path='${path} token=${this.authToken}`)
      ...
      let resp: any = this.authToken
        ? await fetch(url, { headers: {Authorization: `Token ${this.authToken}`} })
        : await fetch(url)
      if (resp.status === 200) {
        resp = await resp.json()
